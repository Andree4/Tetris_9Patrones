...
ABoard::ABoard()
{
    ...
    APieces* PieceA = APieces::GetInstanciaPieces(); // Obtén la instancia de ABloqueBuilderAzul
    APieces* PieceB = APieces::GetInstanciaPieces();
...
void ABoard::BeginPlay()
{
    ...
    Vidas = GetWorld()->SpawnActor<ACreadorVidas>(ACreadorVidas::StaticClass());
    Vid = Vidas->iniciar();
    ...
    while(z < 20)
    {
        ...
        else // this line is full, remove the line
        {
            cont = cont + 1;
             ...
             if (cont >= 8) {
                 GEngine->AddOnScreenDebugMessage(-1, 10, FColor::Blue, TEXT("Eliminado"));
             }
             else {
                 if (cont % 2 ==0) {
                     Vidas->cambiarVida(Vid);
                 }
