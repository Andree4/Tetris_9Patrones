...
//singleton
APieces* APieces::instancia = nullptr;
APieces* APieces::GetInstanciaPieces()
{
	if (instancia == nullptr) { 
		instancia = NewObject<APieces>();
		GEngine->AddOnScreenDebugMessage(-1, 10, FColor::Green, TEXT("se creo instancia a APieces"));
	}
	else {
...
void APieces::RegisterObserver(IObserver* Observer)
{
	Observers.AddUnique(Observer);
}

void APieces::RemoveObserver(IObserver* Observer)
{
	Observers.Remove(Observer);
}

void APieces::NotifyObservers()
{
...
void APieces::SpawnBlocks()
{
    ...
    for (auto&& YZ : YZs)
    {
        ...
        switch (build) {
        case 1:
            B = GetWorld()->SpawnActor<ABloqueBuilderLadrillo>(this->GetActorLocation(), Rotation);
            break;
        case 2:
            B = GetWorld()->SpawnActor<ABloqueBuilderAzul>(this->GetActorLocation(), Rotation);
            break;
        case 3:
            B = GetWorld()->SpawnActor<ABloqueBuilderPasto>(this->GetActorLocation(), Rotation);
            break;
        case 4:
            B = GetWorld()->SpawnActor<ABloqueBuilderLadrillo>(this->GetActorLocation(), Rotation);
            break;
